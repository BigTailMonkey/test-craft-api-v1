...
def generate_ideas(source_code, stream=True, open_ai_api_key="", model=""):  # 根据HTML元素生成测试思路
    ...
    prompt = f"""
        请仔细阅读下面三个反引号中的前端HTML代码：
        HTML:
        ```
        {parse_html(source_code)}
        ```
        根据前端代码中的HTML元素生成测试用例。请像真正的测试人员一样逐步思考：
            - 测试代码在边界值和临界情况下的表现。
            - 模拟可能出现的异常输入或情况，检查代码的容错能力。
            - 专注于用户导向，针对代码的主要功能进行测试，确保能正确实现预期功能。
            - 测试用例要具有可操作性，避免涉及div或class等HTML元素。
            - 识别所有用户可见的输入/输出节点（如：文本输入框、操作按钮、状态提示区）
            - 标记存在业务约束的交互点（如：数值范围限制、格式校验规则）
            - 发现隐式状态依赖（如：步骤顺序依赖、会话有效期）
            - 包含正面测试、负面测试场景，并给出相应的示例（这点特别重要），正面测试和负面测试场景下各不少于15个测试用例。
